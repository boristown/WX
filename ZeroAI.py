...
def chat(input_text):
  ...
  if len(alias_results) == 0:
    ...
    if len(alias_results) == 0:
      ...
      select_alias_statment = "SELECT predictions.*, symbol_alias.SYMBOL_ALIAS FROM symbol_alias inner join predictions on predictions.symbol = symbol_alias.symbol "
        + " inner join (select symbol, max(time) as time from predictions group by symbol) as t1 on predictions.symbol = t1.symbol and predictions.time = t1.time "
        + " WHERE symbol_alias.market_type = '" 
        + input_text 
        + "' AND symbol_alias.market_order > 0 group by symbol ORDER BY time DESC"
