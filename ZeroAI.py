...
def chat(input_text):
  ...
  if len(alias_results) == 1:
    ...
    plt.title( alias_result[2] + ":" + alias_result[0] + " " 
              ...
              + " UTC\n预测结果由AI自动生成Forecast results are automatically generated by AI") #图标题 
    
    #plt.xlabel(u'过去120天收盘价') #X轴标签
    plt.xlabel(u'1天后After 1 day：' + day_prediction_text(predictions_result[2])+
               u'\n关注微信公众号AI纪元，输入Input:' + 
               alias_result[0] + 
               u'\nFollow Wechat Public Account: AI纪元, Input:'+ alias_result[0]) #X轴标签
    ...
    output_text = '1天后After 1 day：' + day_prediction_text(predictions_result[2])
