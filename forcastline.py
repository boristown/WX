...
def draw_market_v2(alias_result, predictions_results, params, origin_input):
    ...
    plt.plot(date_predict,[forcast_price_list[-1]] * int(params["LEN"]), color = "darkviolet", linestyle = "--", label="海龟6号预测Forcast v6:"+str(float(forcast_price_list[-1])))
    plt.plot(date_predict,[forcast_price_list7[-1]] * int(params["LEN"]), color = "darkcyan", linestyle = "--", label="海龟7号预测Forcast v7:"+str(float(forcast_price_list7[-1])))
